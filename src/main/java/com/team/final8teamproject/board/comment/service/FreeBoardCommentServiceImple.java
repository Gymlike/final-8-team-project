...
public class FreeBoardCommentServiceImple implements FreeBoardCommentService {
...
//        t_exerciseService.findT_exerciseBoardById(boardId);
       if (freeBoardRepository.existsById(boardId)) {
           FreeBoardComment freeBoardComment = new FreeBoardComment(comment, userName, boardId,userNickname);
           commentRepository.save(freeBoardComment);
    ...
    public ResponseEntity<String> deleteComment(BaseEntity user, Long commentId) {
        ...
        FreeBoardComment comment = commentRepository.findById(commentId).orElseThrow(()->new CustomException(ExceptionStatus.COMMENT_NOT_EXIST));
    ...
    @Override
    public List<FreeBoardComment> findCommentByBoardId(Long boardId){
